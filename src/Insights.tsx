...
/* =========================
   Distance Distribution (Carry + Roll stacked)
========================= */
// (…existing code unchanged…)

/* ---------- HIGHLIGHTS (ignores filters if allShots provided) ---------- */
// (…existing code unchanged…)
...
// Optimal (PGA Tour) smash factor per club for comparison
...
const benchData = useMemo(() => {
  ...
  // Smash factor metrics
  const smashVals: number[] = [];
  for (const s of shots) {
    let sf: number | null = null;
    if (typeof s.SmashFactor === "number" && Number.isFinite(s.SmashFactor)) {
      sf = s.SmashFactor;
    } else if (typeof s.BallSpeed_mph === "number" && Number.isFinite(s.BallSpeed_mph) &&
               typeof s.ClubSpeed_mph === "number" && Number.isFinite(s.ClubSpeed_mph) && s.ClubSpeed_mph > 0) {
      sf = s.BallSpeed_mph / s.ClubSpeed_mph;
    }
    if (sf != null && Number.isFinite(sf)) smashVals.push(sf);
  }
  const avgSmash = smashVals.length ? avg(smashVals) : null;
  const optSmash = key ? optimalSmash[key] : undefined;
  const smashPct = (avgSmash != null && optSmash != null) ? (avgSmash / optSmash) * 100 : null;
      ...
      {benchData ? (
        <div className="grid grid-cols-2 gap-3">
          ...
          {benchData.avgSmash != null ? (
            <KpiCell theme={T} label="Smash Factor" value={`${benchData.avgSmash.toFixed(2)}`} sub={`n=${benchData.nSmash}`} />
          ) : (
            <KpiCell theme={T} label="Smash Factor" value={"No data"} />
          )}
          <KpiCell
            theme={T}
            label="Smash vs Optimal"
            value={benchData.smashPct != null && benchData.optSmash != null ? `${benchData.smashPct.toFixed(1)}%` : "—"}
            sub={benchData.optSmash != null ? `Optimal ${benchData.optSmash.toFixed(2)}` : undefined}
          />
        </div>
      ) : (
...
/* ---------- RECORDS (All shots; respects filters) ---------- */
// (…existing code unchanged…)

/* ---------- GAPS (ignores filters) ---------- */
// (…existing code unchanged…)
...
export default function Insights(props: Props) {
  ...
  // (…rest of file unchanged: dist, high, swings, records, gaps, progress rendering and cardMap…)
  return (
